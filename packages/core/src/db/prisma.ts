...
export type DBProviders = 'postgres' | 'mysql' | 'cockroach' | 'sqlite' | 'memory';
...
export const getPostgres = (): PostgresClient => {
  if (!postgres) {
    postgres = new PostgresClient({ datasources: { db: { url: config.DB.URL } } });
  }
  return postgres;
};
...
export const getMySQL = (): MySQLClient => {
  if (!mysql) {
    mysql = new MySQLClient({ datasources: { db: { url: process.env.DB_URL_MYSQL } } });
  }
  return mysql;
};
...
export const getCockroach = (): CockroachClient => {
  if (!cockroach) {
    cockroach = new CockroachClient({ datasources: { db: { url: process.env.DB_URL_COCKROACH } } });
  }
  return cockroach;
};
...
export const getSQLite = (): SQLiteClient => {
  if (!sqlite) {
    sqlite = new SQLiteClient({ datasources: { db: { url: process.env.DB_URL_SQLITE } } });
  }
  return sqlite;
};

// ---------- Dynamic Client Getter ----------
...
function addLogging(client: PrismaClients, name: string) {
